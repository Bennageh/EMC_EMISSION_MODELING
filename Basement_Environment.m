...
datas = readmatrix('datas_1.xlsx'); %  551 rows, 7 columns
awgn_data=readmatrix('awgn_1.xlsx');   %  551 rows, 3 columns
...
% NLOS data computation from experimental data
mW_NLOS=datas(1:end,5);
dB_NLOS=datas(1:end,6);

% Transmitted power density at TX antenna (Conducted Emission Measurement)
dB_CE_TX_antenna=datas(1:end,9);
CE_TX_antenna=10.^(dB_CE_TX_antenna/10);    % Conversion to milliWatt
...
% Awgn Basement measuremenmt
awgn_exp=awgn_data(1:end,4);
SNR_dB=mean(dB_RE(194:326)-awgn_exp(194:326));  % SNR in transmission bandwidth
SNR=10^(SNR_dB/10);
% statitical awgn model
A=mean(mW_RE(194:326,1));     % transmit band at rows 194:326
Var=var(mW_RE(194:326,1));
dim_awgn=size(awgn_exp);    % (551,1)
M=dim_awgn(1);
